...
impl TreeSequence {
    ...
    /// # Return
    ///
    /// * `None` if `site` is out of range
    /// * `Some(site)` if `site` is a valid row id.
    ///
    /// # Complexity
    ///
    /// `O(1)`
    ...
    /// Create a tree sequence backed by a pointer from an arbitrary
    /// memory allocator.
    /// This function facilitates using tree sequences in environments where `malloc`
    /// is not the primary allocator.
    /// For example, one can create a tree sequence from a pointer returned by
    /// `PyMem_Malloc`.
    ///
    ...
    /// * The memory allocated to `ptr` **must** be freed by converting the returned tree sequence
    ///   back to a pointer via [``TableCollection::into_mut_ptr``], tearing down the tree
    ///   sequence, and then calling the appropriate method to free the allocation.
    ///
