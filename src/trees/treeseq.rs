    ...
    fn new(ts: &'treeseq TreeSequence, site: SiteId) -> Option<Self> {
        if site.as_usize() < ts.sites().num_rows().as_usize() {
            ts.mutations()
                ...
                .position(|&s| s == site)
                .map_or_else(
                    // there are no mutations at this site, so we return
                    // an empty iterator
                    || {
                        Some(Self {
                            ts,
            ...
            // site is out of range, so we simply return "no instance" at all
            None
        }
    }
}
...
/// The iterator `Item` generated by [`TreeSequence::site_and_mutation_iter`].
pub struct CurrentSite<'treeseq> {
    id: SiteId,
    mutations: MutationIdIterator<'treeseq>,
}

impl<'treeseq> CurrentSite<'treeseq> {
    /// The site table row ([`SiteId`]) of the current site.
    pub fn id(&self) -> SiteId {
        self.id
    }

    /// Consume self and return an iterator over the ['MutationId']
    /// at the current site.
    ///
    /// The order of the output [`MutationId`] is the same as the
    /// order in the parent [`TreeSequence`].
    pub fn into_mutation_iter(self) -> impl Iterator<Item = MutationId> + 'treeseq {
        self.mutations
    }
}
...
impl TreeSequence {
    ...
    /// Iterate over the [`MutationId`] at a specific site.
    ///
    /// # Return
    ///
    /// * `None` if `site` is out of range
    /// * `Some(iterator)` if `site` is a valid row id.
    ///
    /// # Notes
    ///
    /// For sites with zero mutations, the function
    /// returns `Some(iterator)` but the iterator is empty.
    ///
    /// The order of the [`MutationId`] in the iterator
    /// is the same as in the mutation table contained in
    /// `self`.
    ///
    /// # Complexity
    ///
    /// `O(n)` to set up due to having to find the first
    /// [`MutationId`] corresponding to `site`.
    pub fn mutations_at_site_iter<S: Into<SiteId>>(
        &self,
        site: S,
    ) -> Option<impl Iterator<Item = MutationId> + '_> {
        MutationIdIterator::new(self, site.into())
    }

    /// Obtain an iterator over sites in the tree sequence
    /// with the ability to iterate over the [`MutationId`] associated
    /// with each site.
    ///
    /// # Return
    ///
    /// An iterator whose item type is [`CurrentSite`].
    pub fn site_and_mutation_iter(&self) -> impl Iterator<Item = CurrentSite<'_>> {
        SiteMutationCoIterator::new(self)
    }
